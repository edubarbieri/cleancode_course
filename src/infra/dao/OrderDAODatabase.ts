...
export default class OrderDAODatabase implements OrderDAO {
	...
	async getOrderItems(idOrder: number): Promise<OrderItemDTO[]> {
		const orderItemsData = await this.databaseConnection.query("select i.description, oi.quantity, oi.price::float from ccca.order_item oi join ccca.product i on (oi.id_item = i.id) where id_order = $1", [idOrder]);
